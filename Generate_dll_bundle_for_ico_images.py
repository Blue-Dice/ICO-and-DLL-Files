...
# Modify IconFile class to handle both file and BytesIO object
class IconFile:
    def __init__(self, f):
        self.type = 0
        self.entries = []
        ...
        if isinstance(f, BytesIO):
            # BytesIO object
            self._load_from_bytesio(f)
        else:
            # File object
            self._load_from_file(f)

    def _load_from_bytesio(self, f):
        f.seek(0)
        data = f.read()
    ...
    def _parse_data(self, data):
        reserved, self.type, count = struct.unpack('<HHH', data[0:6])

        assert reserved == 0
        assert self.type == 1

        for i in range(0, count):
            self.entries.append(IconDirEntry(data, 6 + i * 16))
...
def main():
    ...
    for folder_name in os.listdir(current_directory):
        ...
        # Convert images to ICO format byte data
        icon_instances = []
        for image_path in image_files:
            with Image.open(image_path) as img:
                # Convert image to byte data
                img_byte_array = BytesIO()
                img.save(img_byte_array, format='ICO')
                img_byte_array.seek(0)
                icon_instances.append(IconFile(img_byte_array))

        # Convert ICO files to DLL
        dll_filename = "IconLib.dll"
        dll_bytes = PeResourceDllFromIconFiles(icon_instances)
